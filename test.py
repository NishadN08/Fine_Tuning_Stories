...
# %%
# Load the dataset
dataset = load_dataset("roneneldan/TinyStories",split = "train[:150000]")
...
prompt = dataset[54]["text"].split(".")[0] + "." + dataset[54]["text"].split(".")[1] + "."
...
# After training — now plot perplexity
train_steps, train_vals = zip(*stepwise_train_loss)

perplexities = [torch.exp(torch.tensor(loss)).item() for loss in train_vals]
# %%
...
with torch.no_grad():
    progress = tqdm(enumerate(val_loader), total=len(val_loader), desc="Validation")
...
avg_val_loss = sum(val_loss_per_batch) / len(val_loss_per_batch)
...
plt.title("Perplexity Comparison: Base vs Fine-Tuned")
